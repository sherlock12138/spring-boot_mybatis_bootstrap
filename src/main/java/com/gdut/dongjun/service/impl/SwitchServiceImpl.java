 ...
 */
@Service
public class SwitchServiceImpl extends BaseServiceImpl<Switch> implements
		SwitchService {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger
			.getLogger(SwitchServiceImpl.class);
	...
	public List<Switch> selectByLineId(String lineId) {
		...
		List<Switch> list = switchMapper.selectByParameters(MyBatisMapUtil
				.warp(map));
		if (logger.isDebugEnabled()) {
	...
	public void delSwitchByLineId(String lineId) {
		...
		List<Switch> switchs = selectByLineId(lineId);// 查找该线路下所有的开关
		for (Switch s : switchs) {// 迭代删除所有开关
			if (s != null)
	...
	public String getAddress(String switchId) {
		Switch switch1 = selectByPrimaryKey(switchId);
		if (switch1 != null) {
	...
	public String getSwitchId(String address) {
		...
		List<Switch> switchs = selectByParameters(MyBatisMapUtil.warp(map));
		
		if (switchs != null && switchs.size() != 0) {
			
			Switch switch1 = switchs.get(0);
			if (switch1 != null) {
